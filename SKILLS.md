...
| **SDK (connected apps)** | `@freeappstore/sdk` (auth, KV, counters, rooms, proxy) | — || **SDK (connected apps)** | `@freeappstore/sdk` (auth, KV, counters, rooms, proxy) | — || **Accent color** | Blue (#2563eb) | Emerald (#10b981) |
...
- Backend (if needed): `@freeappstore/sdk` (auth, KV, counters, rooms, proxy). `npm i @freeappstore/sdk`.
